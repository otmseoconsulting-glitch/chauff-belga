# 01 — Supabase Schema
# Production Database Schema: Tables, Indexes, RPC Functions & RLS Policies
# Project: Chauffagiste-Belga
# Migration file: `supabase/migrations/20260101000000_initial_schema.sql`
...
| Table | Purpose |
|-------|---------|
| `provinces` | 11 Belgian provinces + Brussels-Capital |
| `arrondissements` | 43 Belgian arrondissements |
| `communes` | 580+ municipalities — single source of truth for all geo data |
| `service_categories` | 8 HVAC/plumbing service types |
| `pseo_pages` | Tracks all generated pSEO landing pages |
| `leads` | Customer inquiries from website forms |
| `testimonials` | Customer reviews (Google-imported) |
| `chat_sessions` | Conversational triage sessions & lead pre-qualification |
| `chat_messages` | Individual streaming chat interaction messages |

**Extensions required**: `uuid-ossp`, `postgis` (geo proximity queries)
...
CREATE INDEX idx_arrondissements_province_id ON arrondissements(province_id);
...
CREATE UNIQUE INDEX idx_communes_slug_fr ON communes(slug_fr);
CREATE UNIQUE INDEX idx_communes_slug_nl ON communes(slug_nl);
CREATE UNIQUE INDEX idx_communes_nis_code ON communes(nis_code);

-- GIN index for postal code array containment queries
CREATE INDEX idx_communes_postal_codes ON communes USING GIN(postal_codes);

-- B-tree indexes for foreign key joins
CREATE INDEX idx_communes_province_id ON communes(province_id);
CREATE INDEX idx_communes_arrondissement_id ON communes(arrondissement_id);

-- Partial index for active communes only (used in sitemap generation)
CREATE INDEX idx_communes_active ON communes(slug_fr) WHERE is_active = true;

-- GiST index for geographic proximity queries (PostGIS)
CREATE INDEX idx_communes_geo ON communes USING GIST(
...
CREATE INDEX idx_service_categories_priority ON service_categories(priority_order)
...
CREATE INDEX idx_pseo_pages_commune_id ON pseo_pages(commune_id);
CREATE INDEX idx_pseo_pages_published ON pseo_pages(is_published) WHERE is_published = true;
CREATE INDEX idx_pseo_pages_full_slug ON pseo_pages(full_slug);
...
CREATE INDEX idx_leads_is_urgent ON leads(is_urgent) WHERE is_urgent = true;
CREATE INDEX idx_leads_commune_id ON leads(commune_id);
...
CREATE INDEX idx_testimonials_commune ON testimonials(commune_id)
...
-- 3. Lead statistics for admin dashboard
CREATE OR REPLACE FUNCTION get_lead_stats(period_days INT DEFAULT 30)
RETURNS TABLE(service_type TEXT, total_count BIGINT, urgent_count BIGINT,
              converted_count BIGINT, last_lead_at TIMESTAMPTZ)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT service_type::TEXT, COUNT(*),
...
-- ============================================================
-- TABLE: chat_sessions
-- Tracks visitor conversational triage, safety alerts & lead attribution
-- ============================================================
...
-- ============================================================
-- TABLE: chat_messages
-- Stores individual message exchanges within a session
-- ============================================================
...
-- Holds customer uploaded photos of broken boilers/error codes
-- ============================================================
...
CREATE POLICY "lead_attachments_anon_insert"
ON storage.objects FOR INSERT TO anon
WITH CHECK (bucket_id = 'lead-attachments');

CREATE POLICY "lead_attachments_service_role_all"
...
| Decision | Rationale |
|----------|-----------|
| `postal_codes TEXT[]` on communes | One commune can have multiple postal codes; GIN index enables fast `ANY()` lookup |
| `pseo_pages.full_slug UNIQUE` | Guarantees no duplicate URLs at database level — build-time safety net |
| `SECURITY DEFINER` on RPC functions | Functions run with creator's privileges, safe even when called by anon |
| `is_active` partial indexes | Excludes inactive communes from most query plans — significant perf gain at 580+ rows |
| `GIST` geo index | Enables PostGIS spatial queries; falls back to Haversine calculation if PostGIS unavailable |
